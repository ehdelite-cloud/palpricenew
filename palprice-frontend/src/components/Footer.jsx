...
function Footer({ lang = "ar" }) {
  ...
  const links = {
    ...
    store: {
      ...
      items: [
        // ← إصلاح: كلا الرابطين يذهبان لـ /contact مع param للتوضيح
        { to: "/join", label: lang === "ar" ? "سجّل متجرك"   : "Register Store", note: true },
        { to: "/contact?from=store-login",    label: lang === "ar" ? "دخول المتاجر" : "Store Login",    note: true },
    ...
    <>
      ...
      <footer className="footer" style={{ background: "#0b1120", color: "white", marginTop: "80px" }}>
        ...
        <div className="footer-features-strip" style={{ borderBottom: "1px solid #1e293b" }}>
          <div style={{ maxWidth: "1240px", margin: "auto", display: "flex", gap: "0", padding: "0 24px", flexWrap: "wrap" }}>
            {features.map((f, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "16px 28px", borderLeft: i > 0 ? "1px solid #1e293b" : "none", flex: "1 1 180px" }}>
        ...
        <div className="footer-main-grid">
          ...
          <div className="footer-brand-col">
            <Link to="/" className="footer-brand-link" style={{ display: "flex", alignItems: "center", gap: "8px", textDecoration: "none", marginBottom: "16px" }}>
              <span style={{ background: "linear-gradient(135deg, #16a34a, #22c55e)", borderRadius: "10px", width: "32px", height: "32px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", color: "white", flexShrink: 0 }}>₪</span>
              <span style={{ fontSize: "20px", fontWeight: "900", color: "#22c55e", letterSpacing: "-0.5px", fontFamily: "Cairo, sans-serif" }}>PalPrice</span>
            </Link>
            ...
            <div className="footer-brand-badges" style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.2)", color: "#86efac", padding: "4px 12px", borderRadius: "99px", fontSize: "12px", fontWeight: "600" }}>
          ...
          {Object.values(links).map((section, si) => (
            <div key={si} className="footer-section">
              <h4 className="footer-col-title" onClick={() => toggleSection(si)} style={{ fontSize: "13px", fontWeight: "700", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "16px", margin: "0 0 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                {section.title}
              ...
              <ul className={`footer-ul ${openSection === si ? "open" : ""}`} style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "10px" }}>
                {section.items.map((item, i) => (
                  <li key={i}>
                    <Link to={item.to} style={{ color: item.note ? "#f59e0b" : "#64748b", fontSize: "14px", textDecoration: "none", transition: "color 0.15s", display: "flex", alignItems: "center", gap: "6px" }}
                      onMouseEnter={e => e.currentTarget.style.color = "#22c55e"}
                      onMouseLeave={e => e.currentTarget.style.color = item.note ? "#f59e0b" : "#64748b"}
                    >
                      <span style={{ fontSize: "10px", opacity: 0.5 }}>←</span>
                      {item.label}
                      {/* رسالة صغيرة تحت روابط المتاجر */}
                      {item.note && (
        ...
        {/* Bottom bar */}
        <div style={{ borderTop: "1px solid #1e293b", padding: "18px 24px" }}>
          <div style={{ maxWidth: "1240px", margin: "auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
            ...
            <div style={{ display: "flex", gap: "20px" }}>
              {[
                ...
                <Link key={i} to={l.to} style={{ color: "#334155", fontSize: "12px", textDecoration: "none", transition: "color 0.15s" }}
                  onMouseEnter={e => e.currentTarget.style.color = "#22c55e"}
