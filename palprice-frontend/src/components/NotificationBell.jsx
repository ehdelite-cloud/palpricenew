...
function NotificationBell({ mode = "admin", token, lang = "ar", dropdownSide = "left", notifications = [], setNotifications }) {
  ...
  const API_BASE = mode === "store"
    ? "/api/tickets/notifications/store"
    : mode === "user"
  ...
  const dropdownStyle = (() => {
    ...
    const dropdownWidth = Math.min(window.innerWidth * 0.92, 360);
    const offset = 8;
    const boundedLeft = Math.max(offset, Math.min(rect.left, window.innerWidth - dropdownWidth - offset));
  ...
  function getToken() {
    if (mode === "store") return localStorage.getItem("token");
  ...
  async function fetchNotifications() {
    ...
    try {
      ...
      if (!res.ok) {
        console.warn(`NotificationBell [${mode}]: ${res.status} from ${API_BASE}`);
        return;
      }
      ...
      console.warn(`NotificationBell [${mode}] error:`, err);
    }
  }
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          fontSize: "18px", transition: "all 0.2s", touchAction: "manipulation", pointerEvents: "auto", WebkitTapHighlightColor: "transparent"
        }}
      ...
      {/* Dropdown — fixed لتجاوز أي overflow */}
      {open && (
        <div style={dropdownStyle}>
          ...
          <div style={{ padding: "14px 16px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              {unread > 0 && (
                <span style={{ background: "#ef4444", color: "white", borderRadius: "99px", fontSize: "11px", fontWeight: "700", padding: "1px 7px" }}>
                  {unread}
                </span>
              )}
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{
                        margin: "0 0 3px", fontSize: "13px", lineHeight: 1.5,
